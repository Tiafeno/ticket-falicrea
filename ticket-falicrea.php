...
class TicketSecurity
{
    ...
    private function send_mail($item, $order, $code, $qrcode_url, $for_client_only = false)
    {
        ...
        $item_product_id = $item->get_product_id(); // product id for this order item
        $sujet = "#$order_id - La commande est maintenant terminée - $site_title";
        ...
        $args = [
            ...
            'customer_ph' => $order->get_billing_phone(),
        ...
        $body = $engine->parseFile('order-complete')->render($args);
        $headers[] = 'From: Ma Formation Store <no-reply@maformation-store.net>';
        $to_client = $args['customer_email']; // Get client address email
        ...
        // On envoie un email au formateur, s'il existe
        if ($former_id):
            $this->send_mailtoformer($item_product_id, $args);
        endif;
    }

    private function send_mailtoformer($product_id, $args = [])
    {
        ...
        if (is_email($args['former_email'])) {
            // TODO: Mail for former
            $headers[] = 'From: Ma Formation Store <no-reply@maformation-store.net>';
            $message = $engine->parseFile('order-former-complete')->render($args);
            $objet_email = "Achat de formation: " . $product->get_title();
            // Envoyer le mail au formateur
            wp_mail($args['former_email'], $objet_email, $message, $headers);
        }
    }
}
...
function ticket_admin_menu()
{
    add_options_page('Les tickets', 'Tickets', 'manage_options', 'ticket-security', function () {
        ...
        foreach ($results as $result) {
            ...
            // Ne pas afficher tous les commandes supprimer
            if (empty($order)) {
                // Supprimer la ligne dans la base de donnée
                $wpdb->query("DELETE FROM $table WHERE order_id = $result->order_id"); // Return false on error
                continue;
            }
