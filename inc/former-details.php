...
final class FormerDetailsClass
{
    ...
    public function enqueue_scripts()
    {
        ...
        wp_register_script('xlsx', plugin_dir_url(__FILE__) . '../assets/js/xlsx.full.min.js', [], null, true);
        ...
        wp_enqueue_style('semantic-table', plugin_dir_url(__FILE__) . '../assets/css/table.min.css');
        wp_enqueue_style('semantic-btn', plugin_dir_url(__FILE__) . '../assets/css/button.min.css');
        wp_enqueue_style('semantic-header', plugin_dir_url(__FILE__) . '../assets/css/header.min.css');
        wp_enqueue_style('semantic-icon', plugin_dir_url(__FILE__) . '../assets/css/icon.min.css');
        wp_enqueue_style('semantic-label', plugin_dir_url(__FILE__) . '../assets/css/label.min.css');
    }

    private function getSQLHandlerFilters() {
...
WHERE post.post_type = %s AND meta.meta_value LIKE %s AND meta.meta_key = %s";
        ...
        $filter = $this->getSQLHandlerFilters(); // Filters
        foreach ($product_ids as $product_id) {
            # Get All defined statuses Orders IDs for a defined product ID (or variation ID)
            $results_order_id = $wpdb->get_col("
            ...
            $data_response->order_ids = [];
            // Parcourir tous les commandes trouvee
            foreach ($results_order_id as $index => $order_id) {
                $order = wc_get_order(intval($order_id));
                $items = $order->get_items();
                foreach ($items as $item) {
                    $data = $item->get_data();
                    if (intval($product_id) !== intval($data['product_id'])) continue;
                    $total += ($data['total'] + $data['total_tax']);
                }
    ...
    public function get_product_details()
    {
        ...
        $filter = $this->getSQLHandlerFilters();
